...
public class FileUtil {
    ...
    public static void generateToJava(int type, Object data, String filePath) throws IOException, TemplateException {
        ...
        if (file.exists()) {
            if (!ConfigUtil.getConfiguration().getIsCover()) {
    ...
    private static Template getTemplate(int type) throws IOException {
        switch (type) {
            ...
            case FreemarketConfigUtils.TYPE_GROUP:
                return FreemarketConfigUtils.getInstance().getTemplate("Group.ftl");
            case FreemarketConfigUtils.TYPE_MESSAGE:
                return FreemarketConfigUtils.getInstance().getTemplate("Message.ftl");
            case FreemarketConfigUtils.TYPE_MESSAGE_EN:
                return FreemarketConfigUtils.getInstance().getTemplate("MessageEN.ftl");
            default:
                return null;
        }
    }
